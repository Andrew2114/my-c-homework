...
void addCandidate(Candidate **candidates, int *count, int *capacity, int state, int city, int dist){
    if (*capacity == 0) {
        *capacity = 4;
        candidates[state] = malloc(*capacity * sizeof(Candidate));
        if (candidates[state] == NULL) return;
    }

    if (*count == *capacity){
        *capacity *= 2;
        candidates[state] = realloc(candidates[state], *capacity * sizeof(Candidate));
        if (candidates[state] == NULL) return;
    }
    ...
    for (int state = 0; state < k; state++) {
        ...
        while (current_edge != NULL) {
            ...
            if (owner[neighbour] == -1){
                addCandidate(candidates, &candidates_count[state], &candidates_capacity[state], state, neighbour, distance);
            }
    ...
    while (total_occupied < n) {
        if (candidates_count[current_state] > 0) {
            ...
            for (int i = 1; i < candidates_count[current_state]; i++) {
                if (candidates[current_state][i].dist < candidates[current_state][best_idx].dist) {
                    best_idx = i;
                }
            } 
            ...
            if (owner[chosen_city] == -1) {
                ...
                while (current_edge != NULL) {
                    ...
                    if (owner[neighbour] == -1) {
                        addCandidate(candidates, &candidates_count[current_state], &candidates_capacity[current_state], 
                                    current_state, neighbour, distance);
                    }
                    current_edge = current_edge->next;
                }
            }
